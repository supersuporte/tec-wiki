...
IS
 ...
 -- LOG --
 ---------
 PROCEDURE log(inicio   IN DATE,
               fim      IN DATE,
               processo IN VARCHAR2) AS
 ...
 BEGIN
    ...
    IF fim IS NULL THEN
        ...
        INSERT INTO bilog
             VALUES (sysdate,
                     processo || ' - ' || to_char(inicio));

    ELSE
    
        INSERT INTO bilog
             VALUES (sysdate,
                     processo || ' - ' || to_char(inicio) || ' a ' || to_char(fim));
 ...
 BEGIN
    ...
    log(v_inicio, v_fim, 'Vendas Liquida');
 ...
 BEGIN
   ...
   IF TO_CHAR(v_data, 'MM') = '01' then
       ...
       log(v_data, NULL, 'Cutoff Anual');
   ELSE
       processa_nfs_cutoff(data);   
       log(v_data, NULL, 'Cutoff');
   END IF;
 ...
 BEGIN
    ...
    log(v_inicio, v_fim, 'Vendas Liquida por Produto');
 ...
 BEGIN
   ...
   IF TO_CHAR(v_data, 'MM') = '01' then
       ...
       log(v_data, NULL, 'Cutoff por Produto Anual');
   ELSE
       processa_nfs_cutoff_prod(data);   
       log(v_data, NULL, 'Cutoff por Produto');
   END IF;
